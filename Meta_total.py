...
def plot_function(result, algo):
    ...
    X = plot_samples/1.e6/plot_samples[-1] * 2
    Y = plot_mean
...
def csv_save(folder, name, algo, term):
    ...
    rewards = []
    result = {}
    if "coffee" in name and "episodic" in name:
        for i in range(num, NUM):
            path = "./" \
                   + folder + "/" + name
            in_path = path + '_' + f'{i}' + '/' + algo + '_1'
            print("path", path + '_' + f'{i}' )
...
for v in range(1):
    NUM = 11
    num = 1
    envs = ["window" + Dense, "coffee" + Dense, "soccer" + Dense]
    # env = "coffee" + Dense
    # env =
    results = {}
    for env in envs:
    ...
    for i in results.keys():
        results[i] = results[i] / 3
    plot_function_promp(results, algo)
    ...
    for i in results.keys():
        results[i] = results[i]/3
    plot_function_et3(results, algo)
...
tikzplotlib.save("./data/Meta/" + value + Dense + ".tex")
